<template>
  <el-dialog
    ...
    <el-form-item label="??????[0-???,1-??]" prop="searchType">
      <el-input v-model="dataForm.searchType" placeholder="??????[0-???,1-??]"></el-input>
    </el-form-item>
    ...
    <el-form-item label="????[0-????,1-????,2-????????????]" prop="attrType">
      <el-input v-model="dataForm.attrType" placeholder="????[0-????,1-????,2-????????????]"></el-input>
    </el-form-item>
    <el-form-item label="????[0 - ??,1 - ??]" prop="enable">
      <el-input v-model="dataForm.enable" placeholder="????[0 - ??,1 - ??]"></el-input>
    </el-form-item>
    <el-form-item label="????" prop="catelogId">
      <el-input v-model="dataForm.catelogId" placeholder="????"></el-input>
    </el-form-item>
    <el-form-item label="?????????????;0-? 1-??,?sku???????" prop="showDesc">
      <el-input v-model="dataForm.showDesc" placeholder="?????????????;0-? 1-??,?sku???????"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          showDesc: [
            { required: true, message: '?????????????;0-? 1-??,?sku???????不能为空', trigger: 'blur' }
          ]
        }
      }
    },
